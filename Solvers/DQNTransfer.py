...
class DQNT(AbstractSolver):
    ...
    def _build_model(self):
        ...
        model.add(VGG16(weights="imagenet", include_top=False,input_shape=(input_size[0], input_size[1], 3)))
        
        #set all layers of vgg to not train
        ...
        if os.path.exists('sonic_model_transfer.json'):
            print("Model Found")
            with open('sonic_model_transfer.json', 'r') as model_file:
                loaded_model_json = model_file.read()
                model = model_from_json(loaded_model_json, custom_objects={'tf': tf})
                # load weights into new model
                model.load_weights("sonic_model_transfer.h5")
            print("Model Loaded")
    ...
    def update_target_model(self):
        ...
        now = datetime.now()
        with open("sonic_model_transfer.json", "w") as json_file:
            json_file.write(model_json)
        # serialize weights to HDF5
        date_time = now.strftime("%m-%d-%Y-%H-%M-%S")
        self.model.save_weights("sonic_model_transfer.h5")
        self.model.save_weights(f"sonic_model_transfer_{date_time}.h5")
        print('Model Saved')
        print('Model Saved')
